...
exports.task = ()-> 
    ...
    compile.on 'exit', ()->
        orig_code = FS.readFileSync 'lib/magic-bingo-ball.js', 'utf8'
        ast = jsp.parse(orig_code)
        ast = pro.ast_mangle(ast)
        ast = pro.ast_squeeze(ast)
        final_code =  pro.gen_code(ast)
        HEADER = MU.to_html( PACKAGE_MUSTACHE, read() )
        FS.writeFileSync 'lib/magic-bingo-ball.min.js', "#{HEADER}\n#{final_code}"
        FS.writeFileSync 'magic-bingo-ball.min.js',     "#{HEADER}\n#{final_code}"